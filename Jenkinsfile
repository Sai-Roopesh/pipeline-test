pipeline {
    ...
    stages {
      ...
      stage('Build & Push Docker image') {
        steps {
          // Injects DOCKER_USER so we can tag the image correctly
          withCredentials([usernamePassword(
            ...
            script {
              // Logs in/out in-memory, no config.json written
  ...
  steps {
    withKubeConfig(credentialsId: 'k8s-config') {
      sh '''
        # Print the moment we’re doing this check
        echo "Verification Time: $(date +'%Y-%m-%d %H:%M:%S')"

        echo "Pod ↔ IMAGE ↔ READY ↔ START_TIME"
        ...
        # Optionally repeat the timestamp before showing the service
        echo "Verification Time: $(date +'%Y-%m-%d %H:%M:%S')"
